...
## WORKAROUND for RTC bug
## ----------------------
if [ ! -h /bin/date ]; then
 date 110812002010 >/dev/null
else
 date 201011081200 >/dev/null
fi

## Useful alias
## ------------
if [ ! -h /bin/ps ]; then
 alias ps="/bin/ps -T -A -o uid,pid,ppid,lwp,c,sz,rss,psr,stime,tty,time,rtprio,pri,priority,nice,policy,cmd"
fi
